...
func TestPrepare(t *testing.T) {
	...
	mClient := mockClient{mock: helper}
	c := &Docker{Conn: &mClient}
	i, err := c.Prepare(context.Background(), &workflow.WorkflowAction{Name: "mything"})
	if err != nil {
...
func TestClient_Prepare(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mClient := mockClient{mock: helper}
			c := &Docker{Conn: &mClient}
			i, err := c.Prepare(context.Background(), &workflow.WorkflowAction{Name: "mything", Image: "test"})
			if err != nil {
				switch {
...
func TestClient_Run(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			stringReader := strings.NewReader("i am logs")
			helper := clientMockHelper{
			...
			c := &Docker{Conn: &mClient}
			logs := make(chan []byte)
			err := c.Run(context.Background(), "1234", logs)
			if err != nil {
				switch {
